...
client.on('message',async (msg)=>{
    ...
    if(msg.content.toLowerCase().startsWith(".f")){
        ...
        let command  = msg.content.toLowerCase().split(" ").pop().trim()
        ...
        if(command.toLowerCase().startsWith("reddit")){
            ...
            if(subreddit === ""){
                msg.channel.send("There was an error parsing the arguements. Please make sure there is no space after the comma")
            }
            else{
        ...
        else if(command.toLowerCase().trim()=== "init"){

            db.collection("Guilds").doc(msg.guild.id).collection("Users").doc(msg.author.id).get().then(data=>{
                if(data.exists){
                    msg.reply("You have already initialized your Fletcher Bot profile. Use other commands!")
                }

                else{
                    
                    db.collection("Guilds").doc(msg.guild.id).collection("Users").doc(msg.author.id).set({
                        ...
                        .setColor('#FFD700')
                        .setTitle(`${msg.author.name}'s Fletcher Bot Profile`)
                        .setDescription('Your newly created profile')
                        .addField(`Flurrency: 0`)
    ...
    else if(msg.mentions.has(msg.author)){
        let adindex = Math.floor(Math.random() * (adjectives.length-1) + 1)
        let nounindex = Math.floor(Math.random() * (nouns.length-1) + 1)

        msg.reply(`This ${adjectives[adindex]} ${nouns[nounindex]} is pinging themself`)
    }
...
client.on('guildCreate', (g) => {
    ...
    db.collection("Guilds").doc(g.id).get().then(data=>{
        if(!data.exists){
